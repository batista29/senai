...
-- pizza_id 11 valor 31.33
insert into pedidos values (default, 93, curdate(), curtime(), null);
--id do pedido 33

-- 2 cubanas
select * from pizzas where nome = "Cubana";
-- pizza_id 14 valor 30.85
insert into pedidos values (default, 93, curdate(), curtime(), null);
--id do pedido 34

--inserindo pedido
insert into itens_pedido values (32,11,1,31.33);
insert into itens_pedido values (32,14,2,30.85);
...
--Thamires de Campos Luz"
call set_total_pedido(29);
--pedido_id - 29

--Everton Jose de Souza"
call set_total_pedido(30);
--pedido_id - 30

--Silvia Roberta de Jesus Garcia"
--pedido_id - 32
call set_total_pedido(32);

-- Gatilhos Trigger
-- Gatilhos e precedimentos são semelhantes oque muda é como são executados
-- Procedimentos temos que chama-los.
-- Gatilhos são automaticos antes ou depois de um DML (insert, update, delete)

-- Exemplo de trigger com delimitador
...
delimiter $
create procedure new_pedido_1item(cliente_id int, piz_id int, quantidade int)
begin 
	insert into pedidos values(default, cliente_id, curdate(), curtime(), NULL);
	set @valor = (select valor from pizzas where pizza_id = piz_id); 
	insert into itens_pedido values (last_insert_id(), piz_id, quantidade, @valor);
end $

delimiter ;

call new_pedido_1item (75, 18, 3);
select * from pedidos;