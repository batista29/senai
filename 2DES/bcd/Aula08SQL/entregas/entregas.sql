...
drop view if exists vw_pedidos_itens;
create view vw_pedidos_itens as
...
--Procedure pizzas
drop procedure if exists pedidos_pizza;
delimiter //
create procedure pedidos_pizza()
	begin
	declare erro_sql tinyint default false;
	declare continue handler for sqlexception set erro_sql = true;
	start transaction;

        insert into pedidos values (null,4,curdate(), curtime(), null);

        insert into itens_pedido values (last_insert_id(),1,1, 32.93);
        insert into itens_pedido values (last_insert_id(),3,1, 32.61);
        insert into itens_pedido values (last_insert_id(),5,1, 32.29);
